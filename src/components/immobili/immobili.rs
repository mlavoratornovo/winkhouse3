...
pub fn Immobili(cx: Scope) -> Element {
    ...
    let mut tablerow0:TableRow = TableRow { columns: columns0 };
    rows.push(tablerow0);
    ...
    let mut tablerow1:TableRow = TableRow { columns: columns1 };
    rows.push(tablerow1);
    ...
    let mut tablerow2:TableRow = TableRow { columns: columns2 };
    rows.push(tablerow2);


    let mut pagination:PaginationInfo = PaginationInfo{startPageItem:1, endPageItem:10, itemPages:10, totalItems:80};
    let mut tabledata:TableViewData = TableViewData { items: rows, pagination: pagination };

    cx.render(rsx!(
        div {
            Navbar(&cx, 1)
            FindBarImmobili(cx)
            TableView(&cx, tabledata)
        }
    ))
}